function [lengthList,freqList,timeList,gaborInfo,header,modList,ampList] = getBurstLengthMP_2(analogData,timeVals,thresholdFactor,displayFlag,stimulusPeriodS,baselinePeriodS,burstFreqRangeHz,maxIteration,adaptiveDictionaryParam,dictionarySize,gaborInfo,header,sub_idx)
...
if isempty(gaborInfo)
    ...
    [gaborInfo,header] = getStochasticDictionaryMP3p1(analogData1,timeVals,maxIteration,adaptiveDictionaryParam,dictionarySize,sub_idx);
end
...
ampList = cell(1,numTrials);
octaveToLengthMultiplier = 4/sqrt(2*pi);
for i=1:numTrials
    threshold = thresholdFactor*getMeanBaseline(gaborInfo(i,:,:),timeVals(1),baselinePeriodS,burstFreqRangeHz);
...
%   o = x(:,SCALE);
    ...
    if ~isempty(goodAtomsInTF)
        mtmp = m(goodAtomsInTF);
        x=unique(mtmp(mtmp==max(mtmp)));
        mBLList=cat(2,mBLList,x);
    end
end
mBL=mean(mBLList);
end